...
In some cases, this may actually be TOO flexible, but no attempt is made
to trap this.
...
Any value "N" to the left of the asterix refers to the "Nth" one.  Any
...
ranges.  In a few cases, negative values are appropriate.
...
Note 3: Depending on whether M and W are 0 or nonzero, D means different
...
the previous occurence of either a day of the week, or a certain time of day.

If $dow is defined, the previous occurence of the day of week is returned.
...
should illustrate the use of Date_GetPrev:
...
Is Date::Manip year 2000 compliant?  Yes, but the question is largely
unimportant!  Date::Manip is basically a parser.  It'll parse dates in any
of thousands of different formats (including the 2 digit years that are
going to cause the computer equivalent of doomsday Dec 31, 1999).  But,
...
A better question is is Date::Manip 10,000 compliant.  No.  If you were to
take the current version forward 8000 years, it would cause any number of
problems.  Actually, Date::Manip should break down somewhere around the
year 3000 since at that point the current method of determining leap years
will break down (we'll be off by a day every 3200 years or so).

The best question is for what dates are Date::Manip useful.  It definitely
can't deal with BC dates.  I haven't tested Date::Manip for years between
0001 AD and 0999 AD, but I think it should handle them.  This is completely
untested however!  More important is the fact that Date::Manip is based on
the Gregorian calendar which wasn't adopted until relatively recently.

So, Date::Manip will return accurate results provided you stay within
the years after the Gregorian calendar was adopted and before 3000 AD.
...
Finally, Date::Manip is flexible.  It will allow you to write Year 2000
NON-compliant code if you really want to.
